...
namespace DatingApp.API.Controllers
{
  ...
  public class PhotosController : ControllerBase
  {
    ...
    public async Task<IActionResult> AddPhotoForUser(int userId, [FromForm]PhotoForCreationDto photo)
    {
      ...
      if (!userFromRepo.Photos.Any(u => u.IsMain)) mappedPhoto.IsMain = true;
      ...
      if (await _repo.SaveAll())
      {
        ...
        return CreatedAtRoute("GetPhoto", new { id = mappedPhoto.Id }, photoToReturn);
      }
    ...
    public async Task<IActionResult> DeletePhoto(int userId, int id)
    {
      ...
      var photoFromRepo = await _repo.GetPhoto(id);
      if (photoFromRepo.IsMain) return BadRequest("Cannot remove main photo");
